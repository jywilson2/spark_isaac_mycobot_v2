...
- `side_graze` — contact off the tip-face pad (lateral > `TARGET_MARKER_TIP_FACE_RADIUS_M`, **4 mm**).
- `wrong_side_axis` — flange +Z not aligned with the **outward** normal. The check is now **signed** (`TARGET_MARKER_TOOL_AXIS_TOL_RAD` ≈ **15°** on `axis_out`, not folded), so it rejects both the **side/barrel** contact (`axis_out ≈ 90°`) and the **flipped/back** contact where +Z points toward the center (`axis_out ≈ 180°`). The flipped case is exactly the "marker turns green but touches the EE from the inside/back" defect the old folded check accepted. Outer surface shell tol is **2 mm** (`TARGET_MARKER_SURFACE_CONTACT_OUTER_TOL_M`). The prior ≈35° / 10 mm / 4 mm outer gate produced false greens (axis 25–32°, lat ≈8 mm, tip short of surface).
...
**Authoritative final-pose check:** the live green during motion is only visual; after the hold the **settled** pose is re-classified. A green that settles wrong-side/through/immersed is overridden to `CONTACT_INVALID_SETTLE` → `PLAN_FAIL(invalid_side|immersed)`. Settle also scans YAML collision spheres (`settle_has_side_sphere_hits`): tip-zone ∩ marker is OK; any **side** hit → `MARKER_EE_SIDE_SPHERE` → `PLAN_FAIL(invalid_side)`. A transient green flash never counts as success.
...
**Planning policy:** tip/flange spheres stay **on** until a short oriented standoff (`contact_standoff_m`); only the final axial standoff→pierce segment omits tip spheres (`try_oriented_tip_face_contact`). Tip-omit length is hard-capped at `contact_nudge_max_m` / `contact_via_nudge_max_m` (≈12 mm) — a multi-cm tip-omit fallback (e.g. ≤60 mm) was removed because MotionGen with tip spheres off can swing the EE barrel into the marker from the side/back. Larger gaps must be closed by a spheres-ON approach (or via); long tip-omit → `contact_nudge_direct_refused`. Skip-near starts that are not pad-aligned reseat with spheres ON before tip-omit.
...
**EE-only + tip-face pad:** Success also requires the middle of the EE tip contact pad on the marker **surface** (`classify_tip_contact`) and **no proximal arm capsule** intersecting the marker (`proximal_arm_contacts_target` → `PLAN_FAIL(arm_body_contact)`). Mid-path `side_graze` / `wrong_side_axis` latch settle `PLAN_FAIL(invalid_side)` (`CONTACT_INVALID_MIDPATH_GRAZE`). See `spec.md` § EE-only surface contact and § **Mandatory contact failure conditions (frozen for experiments)** — those failure rows must not be removed in future experiments.

**Countable episodes + skip gate:** `--visualize N` fills N planned episodes; `SKIPPED_UNREACHABLE` does not consume a slot. Smoke fails when `skip_unreachable/candidates > max_skip_unreachable_frac` (default 0.25).
