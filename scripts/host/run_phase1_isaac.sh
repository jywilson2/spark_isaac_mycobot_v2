#!/usr/bin/env bash
# Host Phase 1: DLS IK metrics **with** Isaac Sim visualization in one run.
#
#   ./scripts/download_mycobot_ros2.sh
#   ./scripts/host/run_phase1_isaac.sh
#
# Quick visual smoke test (240 metric poses ≈ one per workspace bin, animate 48):
...
if [[ -f /.dockerenv && "${SPARK_ALLOW_CONTAINER_ISAAC:-0}" != "1" ]]; then
  echo "Phase 1 metrics with Isaac Sim rendering must run on the **host**." >&2
  echo "  ${SPARK_REPO_ROOT}/scripts/host/run_phase1_isaac.sh" >&2
...
# Defaults for the unified Isaac run (overridable via env or forwarded flags)
N_POSES="${PHASE1_N_POSES:-1000}"
N_VIZ="${PHASE1_VISUALIZE:-48}"
SEED="${PHASE1_SEED:-0}"

if [[ "${SKIP_TESTS}" -eq 0 ]]; then
  echo "=== Phase 1 unit tests (host python) ==="
...
LOG_PATH="$(spark_host_new_log phase1_isaac_metrics_viz)"
echo "=== Phase 1 metrics + Isaac Sim visualization ==="
...
spark_host_run_python \
  "${SPARK_REPO_ROOT}/isaac_sim/run_phase1_ik_viz.py" \
...
if [[ "${exit_code}" -ne 0 ]]; then
  echo "Phase 1 metrics+viz FAILED (exit ${exit_code})" >&2
  spark_host_print_log_tail "${LOG_PATH}" 80
  exit "${exit_code}"
fi

echo "Phase 1 host run complete (metrics written during Isaac session)."
